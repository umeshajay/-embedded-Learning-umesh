...
function makeFoundationQuestion(id, levelIndex, n, difficulty) {
  ...
  const r = (n % 12);
  const fb = (q, ans, hint) => ({ id, q, a: ans, hint });

  if (easy) {
    switch (levelIndex) {
      case 0:
        if (r === 0) return fb(`0b${a.toString(2)} in decimal = ?`, a, `Binary ${a.toString(2)} = ${a} decimal.`);
        if (r === 1) return fb(`0x${a} in decimal = ?`, a, `Hex 0x${a} = ${a} decimal.`);
        if (r === 2) return fb(`${a} bytes = ? bits`, a * 8, "1 byte = 8 bits.");
        if (r === 3) return fb(`Decimal ${a + b} in hex? (enter decimal value)`, a + b, `Just the decimal value of the hex.`);
        if (r === 4) return fb(`0xFF in decimal = ?`, 255, "15*16 + 15 = 255.");
        if (r === 5) return fb(`${a} KB = ? bytes`, a * 1024, "1 KB = 1024 bytes.");
        if (r === 6) return fb(`uint8 max value = ?`, 255, "2^8 - 1 = 255.");
        if (r === 7) return fb(`0b1111 in decimal = ?`, 15, "8+4+2+1 = 15.");
        if (r === 8) return fb(`${a} + ${b} = ? (register add)`, a + b, "Add register values.");
        if (r === 9) return fb(`${a + b} - ${a} = ? (register sub)`, b, "Subtract.");
        if (r === 10) return fb(`4 bits max value = ?`, 15, "1111 = 15.");
        return fb(`1 byte can store ? values`, 256, "0 to 255 inclusive.");
      case 1:
        if (r === 0) return fb(`V = IR, I=${a}, R=${b}. V = ?`, a * b, "V = I x R.");
        if (r === 1) return fb(`f = 1/T, T=${a}s. f = ? (Hz)`, +(1 / a).toFixed(3), `f = 1/${a}.`);
        if (r === 2) return fb(`x + ${a} = ${a + b}, x = ?`, b, `Subtract ${a}.`);
        if (r === 3) return fb(`${a}x = ${a * b}, x = ?`, b, `Divide by ${a}.`);
        if (r === 4) return fb(`V = ${a} * R, R=${b}. V = ?`, a * b, `V = ${a} x ${b}.`);
        if (r === 5) return fb(`x / ${a} = ${b}, x = ?`, a * b, `Multiply by ${a}.`);
        if (r === 6) return fb(`T = 1/f, f=${a}Hz. T = ? s`, +(1 / a).toFixed(3), `T = 1/${a}.`);
        if (r === 7) return fb(`y = ${a}x, x=${b}. y = ?`, a * b, `y = ${a} x ${b}.`);
        if (r === 8) return fb(`x - ${a} = ${b}, x = ?`, a + b, `Add ${a}.`);
        if (r === 9) return fb(`I = V/R, V=${a * b}, R=${b}. I = ?`, a, `I = ${a*b}/${b}.`);
        if (r === 10) return fb(`x + ${a} = ${b + a}, x = ?`, b, `Subtract ${a}.`);
        return fb(`P = IV, I=${a}, V=${b}. P = ?`, a * b, "P = I x V.");
      case 2:
        if (r === 0) return fb(`ADC: Vout = (${a}/${b}) x Vref. If Vref=1V, Vout = ?`, +(a / b).toFixed(2), `Vout = ratio x Vref.`);
        if (r === 1) return fb(`y = ${a}x + ${b}, x=0 => y = ?`, b, "y-intercept.");
        if (r === 2) return fb(`Sensor: y = ${a}x, x=${b}. y = ?`, a * b, `y = ${a} x ${b}.`);
        if (r === 3) return fb(`Slope of y = ${a}x is ?`, a, "Coefficient of x.");
        if (r === 4) return fb(`y = ${a}x, x doubles => y ?`, 2, "y doubles too (linear).");
        if (r === 5) return fb(`f(x) = ${a}x + ${b}, f(${c}) = ?`, a * c + b, `Plug x = ${c}.`);
        if (r === 6) return fb(`Vout = Vin x (${a}/${b}), Vin=${b}. Vout = ?`, a, `${a}/${b} x ${b} = ${a}.`);
        if (r === 7) return fb(`Scaling: ${a} x ${b} = ?`, a * b, "Multiply.");
        if (r === 8) return fb(`${a}x = ${a * b}, x = ?`, b, `Divide by ${a}.`);
        if (r === 9) return fb(`y = ${a}x + ${b}, slope = ?`, a, "Coefficient of x.");
        if (r === 10) return fb(`ADC count ${a}, 10-bit max 1023. Approx % = ?`, Math.round(a / 1023 * 100), `(${a}/1023) x 100.`);
        return fb(`Offset: y = x + ${b}, when x=0, y = ?`, b, "Offset term.");
      case 3:
        if (r === 0) return fb(`uint8 array[${a}]: bytes = ?`, a, `${a} x 1 byte.`);
        if (r === 1) return fb(`int16 array[${a}]: bytes = ?`, a * 2, `${a} x 2 bytes.`);
        if (r === 2) return fb(`${a} KB SRAM = ? bytes`, a * 1024, "1 KB = 1024 B.");
        if (r === 3) return fb(`int32 array[${a}]: bytes = ?`, a * 4, `${a} x 4 bytes.`);
        if (r === 4) return fb(`Register 0x${a}${b}: hex value = ? (decimal)`, a * 16 + b, `${a}*16+${b}.`);
        if (r === 5) return fb(`${a} MB flash = ? KB`, a * 1024, "1 MB = 1024 KB.");
        if (r === 6) return fb(`uint16 max = ?`, 65535, "2^16 - 1.");
        if (r === 7) return fb(`float32 array[${a}]: bytes = ?`, a * 4, `${a} x 4 bytes.`);
        if (r === 8) return fb(`Address 0x${a}00 = ? (decimal)`, a * 256, `${a} x 256.`);
        if (r === 9) return fb(`${a} byte(s) can address ? values`, 2 ** (a * 8), `${a} bytes = ${a * 8} bits.`);
        if (r === 10) return fb(`int8 max = ?`, 127, "2^7 - 1.");
        return fb(`int8 min = ?`, -128, "-2^7.");
      case 4:
        if (r === 0) return fb(`sin(0) for AC signal = ?`, 0, "Crossing zero.");
        if (r === 1) return fb(`f = ${a}0 Hz, T = ? ms`, Math.round(1000 / (a * 10)), "T = 1000/f ms.");
        if (r === 2) return fb(`cos(0) for AC signal = ?`, 1, "Peak value.");
        if (r === 3) return fb(`f = ${a}0 Hz, period = ? ms`, Math.round(1000 / (a * 10)), "T = 1000/f ms.");
        if (r === 4) return fb(`PWM duty ${a * 10}%, ${a * 10}us period: ON time = ?`, a * 10 * (a * 10) / 100, `duty% x period / 100.`);
        if (r === 5) return fb(`sin(90) for sine wave = ?`, 1, "Peak at 90 degrees.");
        if (r === 6) return fb(`cos(90) = ?`, 0, "Zero crossing.");
        if (r === 7) return fb(`1 cycle = ? degrees`, 360, "360 degrees.");
        if (r === 8) return fb(`sin(180) = ?`, 0, "Zero crossing.");
        if (r === 9) return fb(`cos(180) = ?`, -1, "Negative peak.");
        if (r === 10) return fb(`Right angle motor step = ? degrees`, 90, "90 degrees.");
        return fb(`PWM period ${a * 10}ms: freq = ? Hz`, Math.round(1000 / (a * 10)), "f = 1000/T.");
      case 5:
        if (r === 0) return fb(`2^${a} = ? (memory boundary)`, 2 ** a, `Power of 2 = ${2 ** a}.`);
        if (r === 1) return fb(`2^10 = 1 ?`, 1024, "1024 = 1K.");
        if (r === 2) return fb(`10^${a} = ? (metric prefix)`, 10 ** a, `10^${a}.`);
        if (r === 3) return fb(`2^${a} bytes = ? (answer as integer)`, 2 ** a, `${2 ** a} bytes.`);
        if (r === 4) return fb(`MHz = 10^? Hz`, 6, "1 MHz = 10^6 Hz.");
        if (r === 5) return fb(`2^0 = 1 byte = ? bits`, 8, "1 byte = 8 bits.");
        if (r === 6) return fb(`GHz = 10^? Hz`, 9, "1 GHz = 10^9 Hz.");
        if (r === 7) return fb(`Next power of 2 after ${a} is ?`, 2 ** Math.ceil(Math.log2(a + 1)), `Round up to next 2^n.`);
        if (r === 8) return fb(`1 ms = 10^? s`, -3, "milli = 10^-3.");
        if (r === 9) return fb(`1 uA = 10^? A`, -6, "micro = 10^-6.");
        if (r === 10) return fb(`2^3 = ? (common register bit count)`, 8, "8 bits in a byte.");
        return fb(`16-bit = 2^?`, 16, "16 bits.");
      case 6:
        if (r === 0) return fb(`Sensor ${a} to ${a + b}: delta = ?`, b, `${a + b} - ${a}.`);
        if (r === 1) return fb(`Speed ${a} m/s for ${b}s: distance = ?`, a * b, "d = v x t.");
        if (r === 2) return fb(`ADC ${a} now, ${a + b} next: delta = ?`, b, `${a + b} - ${a}.`);
        if (r === 3) return fb(`f(x) = ${a}x (linear sensor). f(${b}) = ?`, a * b, `f(${b}) = ${a} x ${b}.`);
        if (r === 4) return fb(`${a} mA for ${b} hours: mAh = ?`, a * b, "mAh = mA x hours.");
        if (r === 5) return fb(`Temp ${a}C to ${a + b}C: rise = ?`, b, `${a + b} - ${a}.`);
        if (r === 6) return fb(`Rate: ${a * 5} bits / ${a} sec = ? bps`, 5, "Bits per second.");
        if (r === 7) return fb(`Const function f(x)=${a}: slope = ?`, 0, "Constant = zero slope.");
        if (r === 8) return fb(`f(x) = ${a}x, f(2) = ?`, a * 2, `f(2) = ${a} x 2.`);
        if (r === 9) return fb(`${a}V to 0V in ${a}s: slew rate = ? V/s`, -1, `${0 - a}/${a}.`);
        if (r === 10) return fb(`f(x) = ${a}x, f(1) + f(3) = ?`, a * 4, `${a} + ${a * 3}.`);
        return fb(`Signal ${a} to ${a * 2}: gain = ?`, 2, `Doubled = 2x.`);
      default:
        return fb(`${a} + ${b} = ?`, a + b, "Add.");
    }
  } else if (advanced) {
    switch (levelIndex) {
      case 0:
        if (r === 0) return fb(`Mask reg &= ~(${a} << ${b}): bit ${b} is ?`, 0, "Cleared to 0.");
        if (r === 1) return fb(`0x${a}${b} + 0x${c} = ? (decimal)`, a * 16 + b + c, `Hex add: ${a * 16 + b} + ${c}.`);
        if (r === 2) return fb(`uint8_t x=${250 + a}; x+=${b}; x = ?`, (250 + a + b) % 256, "uint8 wraps at 256.");
        if (r === 3) return fb(`${a} KB = ? bytes (sRAM budget)`, a * 1024, "1 KB = 1024 B.");
        if (r === 4) return fb(`0x${a}${b} & 0xF0 = ? (enter decimal)`, a * 16, `Mask low nibble: ${a * 16 + b} & 0xF0.`);
        if (r === 5) return fb(`reg |= (1 << ${a}): bit ${a} set. reg = reg | ?`, 2 ** a, `OR with ${2 ** a}.`);
        if (r === 6) return fb(`Q8.8: ${a} in Q8.8 = ? (enter integer)`, a * 256, "Q8.8 scale = 256.");
        if (r === 7) return fb(`Little-endian: 0x${a}${b}${c}${d} low byte = ?`, d, "Lowest byte first.");
        if (r === 8) return fb(`2^${a} - 1 = ? (all-ones mask)`, 2 ** a - 1, `${a} bits all set = ${2 ** a - 1}.`);
        if (r === 9) return fb(`Store ${a}.${b} as int16 scale 100: ?`, a * 100 + b, "Multiply by 100.");
        if (r === 10) return fb(`Overflow: int8 ${120 + a} + ${b} = ?`, (120 + a + b) << 24 >> 24, "int8 overflow wraps.");
        return fb(`reg = 0x${a}${b}. Set bit 0: reg = ? (decimal)`, (a * 16 + b) | 1, `OR with 1: ${a * 16 + b} | 1.`);
      case 1:
        if (r === 0) return fb(`Baud: BR = Fclk / (16 * UBBR). Fclk=${a}M, UBBR=${b}: BR = ?`, Math.round(a * 1e6 / (16 * b)), `Fclk / (16 * UBBR).`);
        if (r === 1) return fb(`Vdiv: Vout = Vin x R2/(R1+R2). Vin=5V, R1=${a}k, R2=${b}k: Vout = ?`, +(5 * b / (a + b)).toFixed(2), `5 x ${b}/(${a}+${b}).`);
        if (r === 2) return fb(`${a}(x + ${b}) = ${a * (c + b)}, x = ?`, c, `Divide by ${a}, subtract ${b}.`);
        if (r === 3) return fb(`Timer: f_timer = f_clk / (prescaler x period). f_clk=${a}MHz, presc=${b}, period=${c}: f = ?`, +(a * 1e6 / (b * c) / 1e3).toFixed(1) + " kHz", `f_clk / (${b} x ${c}).`);
        if (r === 4) return fb(`Batt: ${a * 100}mAh, draw ${b}mA: hours = ?`, (a * 100) / b, `${a * 100} / ${b}.`);
        if (r === 5) return fb(`V = IR, I=${a}mA, R=${b}kOhm. V = ?`, a * b, `V = ${a}m x ${b}k = ${a * b}V.`);
        if (r === 6) return fb(`${a}x + ${b}x = ${(a + b) * c}, x = ?`, c, `Combine: ${a + b}x.`);
        if (r === 7) return fb(`P = IV, I=${a}mA, V=${b}V. P = ? mW`, a * b, `${a}m x ${b} = ${a * b}mW.`);
        if (r === 8) return fb(`RC tau = R x C. R=${a}k, C=${b}uF. tau = ? ms`, a * b, `${a}k x ${b}u = ${a * b}ms.`);
        if (r === 9) return fb(`f = 1/(2pi RC). R=${a}k, C=${b}uF, pi=3. f = ? Hz`, Math.round(1 / (2 * 3 * a * b * 1e-3)), `1/(6 x ${a}k x ${b}u).`);
        if (r === 10) return fb(`${a}x - ${b} = ${a * c - b}, x = ?`, c, `Add ${b}, divide by ${a}.`);
        return fb(`Duty% = compare/period x 100. comp=${a}, period=${b}: duty = ?%`, Math.round(a / b * 100), `(${a}/${b}) x 100.`);
      case 2:
        if (r === 0) return fb(`ADC: V = count x Vref / 2^n. count=${a}, Vref=3.3V, 10-bit: V = ?`, +(a * 3.3 / 1023).toFixed(2), `(${a} x 3.3) / 1023.`);
        if (r === 1) return fb(`Sensor lin: y = ${a}x + ${b}, x=${c}. y = ?`, a * c + b, `Substitute x=${c}.`);
        if (r === 2) return fb(`Normalize: z = (x - mean)/std. x=${c * 2}, mean=${c}, std=${a}: z = ?`, +(c / a).toFixed(2), `(${c * 2} - ${c}) / ${a}.`);
        if (r === 3) return fb(`Cal: y = mx + b. m=${a}, b=${b}. x=${c}: y = ?`, a * c + b, `y = ${a}(${c}) + ${b}.`);
        if (r === 4) return fb(`Perf slope to y = ${a}x + ${b} = ?`, -1 / a, "Negative reciprocal.");
        if (r === 5) return fb(`Quantize: q = round(x/scale). x=${a * 10}, scale=${a}: q = ?`, 10, `${a * 10}/${a}.`);
        if (r === 6) return fb(`f(x) = ${a}x + ${b}, g(x) = ${c}x. f(${d}) + g(${d}) = ?`, (a + c) * d + b, `Add: ${a}(${d}) + ${b} + ${c}(${d}).`);
        if (r === 7) return fb(`Conv: input ${a}ch, kernel ${b}x${b}, stride 1: output = ?`, a - b + 1, `n - k + 1 = ${a} - ${b} + 1.`);
        if (r === 8) return fb(`Dequant: x = scale x (q - zp). scale=0.${a}, q=${b}, zp=0: x = ?`, +(0.1 * a * b).toFixed(2), `0.${a} x ${b}.`);
        if (r === 9) return fb(`Softmax numerator: e^${a}. Roughly = ? (e=2.72)`, Math.round(Math.E ** a), `e^${a} approx ${Math.round(Math.E ** a)}.`);
        if (r === 10) return fb(`x + y = ${a + b}, x - y = ${a - b}. x = ?`, a, "Add equations.");
        return fb(`MSE: errors [${a}]. Square each, mean = ?`, a * a, `(${a}^2) / 1 = ${a * a}.`);
      case 3:
        if (r === 0) return fb(`Model weights: ${a * 1000} params, 2 bytes each = ? KB`, (a * 1000 * 2) / 1024, `(${a}K x 2)/1024.`);
        if (r === 1) return fb(`Frame buf: ${a * 10}x${b * 10} RGB565 (2B/pixel) = ? KB`, (a * 10 * b * 10 * 2) / 1024, `(${a * 10} x ${b * 10} x 2) / 1024.`);
        if (r === 2) return fb(`Conv layer: ${a} input ch, ${b} output ch, ${c}x${c} kernel = ? params`, (a * b * c * c) + b, `(${a} x ${b} x ${c}^2) + ${b} biases.`);
        if (r === 3) return fb(`Address range: 0x${a}000 to 0x${a}FFF = ? bytes`, 4096, `${a}FFF - ${a}000 + 1 = 4096.`);
        if (r === 4) return fb(`int8 weights ${a}K: RAM needed = ? KB`, a, "int8 = 1 byte each.");
        if (r === 5) return fb(`Tensor: ${a}x${b}x${c} float32 = ? KB`, (a * b * c * 4) / 1024, `(${a} x ${b} x ${c} x 4) / 1024.`);
        if (r === 6) return fb(`SRAM: 0x${a}00 to 0x${a}FF = ? bytes`, 256, "256 bytes.");
        if (r === 7) return fb(`Dense layer: ${a} inputs, ${b} outputs = ? params`, a * b + b, `(${a} x ${b}) + ${b} biases.`);
        if (r === 8) return fb(`Flash: 2^${a} bytes = ? KB`, 2 ** a / 1024, `${2 ** a} / 1024 = ${2 ** a / 1024} KB.`);
        if (r === 9) return fb(`Stride ${a} on ${b * a}x${c} buf: output width = ?`, Math.ceil((b * a) / a), `${b * a} / ${a}.`);
        if (r === 10) return fb(`Pad ${a} on ${b}x${b} -> ? x ?`, b + 2 * a, `${b} + 2*${a}.`);
        return fb(`int16 array[${a * 100}]: bytes = ?`, a * 100 * 2, `${a * 100} x 2.`);
      case 4:
        if (r === 0) return fb(`BLDC: 3 phases, each 120 deg apart. 360/3 = ?`, 120, "360/3 = 120 degrees.");
        if (r === 1) return fb(`FFT bin ${a}: real=${b}, imag=0. Magnitude = ?`, b, "Magnitude = sqrt(real^2).");
        if (r === 2) return fb(`sin(30)=0.5, cos(30)=0.87. tan(30)=?`, +(0.5 / 0.87).toFixed(2), "sin/cos.");
        if (r === 3) return fb(`PWM fund freq ${a * 10}Hz, 3rd harmonic = ? Hz`, a * 10 * 3, "3x fundamental.");
        if (r === 4) return fb(`IQ rotation: I=${a}, Q=0, angle 90 deg. Rotated Q = ?`, a, "Q = I after 90 deg.");
        if (r === 5) return fb(`AC: Vpk=${a}V. Vrms = ? (Vpk/sqrt2)`, +(a / 1.414).toFixed(2), "Vpk / sqrt2.");
        if (r === 6) return fb(`sin^2(θ) + cos^2(θ) = ? for any θ`, 1, "Pythagorean identity.");
        if (r === 7) return fb(`Clarke: Ia=${a}, Ib=${b}. Ialpha = Ia = ?`, a, "Ialpha = Ia.");
        if (r === 8) return fb(`sin(2 x ${a * 10}) = sin(?)`, 2 * a * 10, `Double angle.`);
        if (r === 9) return fb(`FFT bin ${a} at ${a * 10}Hz. Bin ${a * 2} = ? Hz`, a * 2 * 10, `Bin k at k x ${a * 10}Hz.`);
        if (r === 10) return fb(`3-phase: Va=${a}, Vb=${b}, Vc=0. Neutral sum = ?`, a + b, "Va + Vb + Vc.");
        return fb(`cos(60)=0.5. sin(60)=? (enter decimal)`, +(Math.sqrt(3) / 2).toFixed(2), "sqrt(3)/2 ≈ 0.866.");
      case 5:
        if (r === 0) return fb(`Model: ${a} layer(s), ${b * 10} params each = ? total`, a * b * 10, `${a} x ${b * 10}.`);
        if (r === 1) return fb(`log2(${2 ** (a + b)}) = ?`, a + b, `2^${a + b}.`);
        if (r === 2) return fb(`Quant scale S = range/256. range=${a * 10}: S = ?`, +(a * 10 / 256).toFixed(3), `${a * 10}/256.`);
        if (r === 3) return fb(`2^${a} x 2^${b} = 2^? (memory add)`, a + b, "Add exponents for multiply.");
        if (r === 4) return fb(`Params = ${a}K x 2 bytes = ? KB`, a * 2, `${a} x 2.`);
        if (r === 5) return fb(`(2^${a})^${b} = 2^? (dimensions)`, a * b, "Multiply exponents.");
        if (r === 6) return fb(`Softmax: e^${a} / (e^${a} + e^${b}) approx = ?`, +(Math.E ** a / (Math.E ** a + Math.E ** b)).toFixed(2), `${Math.E ** a}/(${Math.E ** a}+${Math.E ** b}).`);
        if (r === 7) return fb(`log2(${2 ** a}) + log2(${2 ** b}) = ?`, a + b, "log(xy) = log(x) + log(y).");
        if (r === 8) return fb(`Arena: ${a}K for activations, ${b}K for weights = ? K total`, a + b, `${a} + ${b}.`);
        if (r === 9) return fb(`Feature vec ${a}-dim, ${b} such vecs = ? total`, a * b, `${a} x ${b}.`);
        if (r === 10) return fb(`2^${a} / 2^${b} = 2^? (shr scale)`, a - b, "Subtract exponents for divide.");
        return fb(`Bias params for ${a} neurons = ?`, a, "1 bias per neuron.");
      case 6:
        if (r === 0) return fb(`P = Kp x e. Kp=${a}, error=${b}. P term = ?`, a * b, `${a} x ${b}.`);
        if (r === 1) return fb(`I += Ki x e x dt. Ki=${a}, e=${b}, dt=1. I accum = ?`, a * b, `${a} x ${b} x 1.`);
        if (r === 2) return fb(`D = Kd x (e - e_prev). Kd=${a}, e=${b}, e_prev=${c}. D = ?`, a * (b - c), `${a} x (${b} - ${c}).`);
        if (r === 3) return fb(`Grad step: w -= lr x dw. lr=0.${a}, dw=${b}. w -= ?`, +(0.1 * a * b).toFixed(2), `0.${a} x ${b}.`);
        if (r === 4) return fb(`Error changes ${b} in ${a}ms. dError/dt = ?`, +(b / a).toFixed(2), `${b}/${a}.`);
        if (r === 5) return fb(`Exponential mov avg: y = 0.${a} x ${b} + 0.${9 - a} x prev. y ~ ?`, +(0.1 * a * b).toFixed(1), `0.${a} x ${b}.`);
        if (r === 6) return fb(`Integral windup: sum errors ${a + b + c} clamped to ${a * 5}: output = ?`, Math.min(a + b + c, a * 5), `Clamp at ${a * 5}.`);
        if (r === 7) return fb(`Accel: ${a} to ${b} in ${c}s. Avg accel = ?`, +(b - a) / c, `(${b} - ${a}) / ${c}.`);
        if (r === 8) return fb(`Loss: y=${a}, y_pred=${b}. MSE = (y - y_pred)^2 = ?`, (a - b) ** 2, `(${a} - ${b})^2 = ${(a - b) ** 2}.`);
        if (r === 9) return fb(`lr schedule: 0.${a} / ${b} = ? (decay)`, +(0.1 * a / b).toFixed(4), `0.${a} / ${b}.`);
        if (r === 10) return fb(`EMA: alpha=0.${a}, new val=${b}, prev=${a * 3}. EMA = ?`, Math.round(0.1 * a * b + (1 - 0.1 * a) * a * 3), `0.${a} x ${b} + 0.${9-a} x ${a * 3}.`);
        return fb(`Batt: ${a * 100}mAh, avg draw ${b}mA. Runtime = ? h`, (a * 100) / b, `${a * 100} / ${b} h.`);
      default:
        return fb(`${a} + ${b} = ?`, a + b, "Add.");
    }
  } else {
    switch (levelIndex) {
      case 0:
        if (r === 0) return fb(`Set bit ${a}: reg |= 1 << ${a}. reg = reg | ?`, 2 ** a, `OR with ${2 ** a}.`);
        if (r === 1) return fb(`Clear bit ${a}: reg &= ~(1 << ${a}). AND with ?`, ~(2 ** a), `AND with NOT(${2 ** a}).`);
        if (r === 2) return fb(`reg & 0x0${a}: mask lower nibble = ? (decimal)`, a, `Mask = ${a}.`);
        if (r === 3) return fb(`${a}KB + ${b}KB = ? KB (total SRAM)`, a + b, "Add capacities.");
        if (r === 4) return fb(`0x${a}${b} in decimal = ?`, a * 16 + b, `${a}*16 + ${b}.`);
        if (r === 5) return fb(`0b${a.toString(2)}${b.toString(2)} (concat) in decimal?`, parseInt(a.toString(2) + b.toString(2), 2), "Concatenate binary strings.");
        if (r === 6) return fb(`uint16_t x=${a * 1000}; x>>8 = ? (high byte)`, Math.floor(a * 1000 / 256), `Right shift by 8.`);
        if (r === 7) return fb(`reg = 0x${a}${b}. reg | 0x0${c} = ? (hex decimal)`, a * 16 + b + c, `Set low nibble: add ${c}.`);
        if (r === 8) return fb(`Scale ${a}.${b} fixed-point (x10): store as ?`, a * 10 + b, `Multiply by 10.`);
        if (r === 9) return fb(`${a} << ${b} = ? (shift left)`, a * (2 ** b), `Multiply by 2^${b}.`);
        if (r === 10) return fb(`reg ^= (1 << ${a}): toggle bit ${a}. XOR with ?`, 2 ** a, `XOR with ${2 ** a}.`);
        return fb(`0x${a}${b} + 0x${c} = ? (decimal)`, a * 16 + b + c, `Add hex: ${a * 16 + b} + ${c}.`);
      case 1:
        if (r === 0) return fb(`Vdiv: Vout = Vin x R2/(R1+R2). Vin=3.3V, R1=${a}K, R2=${b}K: Vout = ?`, +(3.3 * b / (a + b)).toFixed(2), `3.3 x ${b}/(${a}+${b}).`);
        if (r === 1) return fb(`Timer: f_timer = ${a}MHz / (prescaler=${b}) = ? kHz`, Math.round(a / b * 1000), `${a}M / ${b}.`);
        if (r === 2) return fb(`${a}x + ${b} = ${a * c + b}, x = ?`, c, `Subtract ${b}, divide by ${a}.`);
        if (r === 3) return fb(`y = ${a}x + ${b}. y-intercept = ?`, b, "Constant term.");
        if (r === 4) return fb(`Ratio ${a}:${b} = ${a * d}:?`, b * d, `Scale by ${d}.`);
        if (r === 5) return fb(`P = IV, I=${a}mA, V=${b}V. P = ? mW`, a * b, `${a}m x ${b} = ${a * b}mW.`);
        if (r === 6) return fb(`x/2 + ${a} = ${a + 3}, x = ?`, 6, `Subtract ${a}, multiply by 2.`);
        if (r === 7) return fb(`${a}(x - ${b}) = ${a}, x = ?`, b + 1, `Divide by ${a}, add ${b}.`);
        if (r === 8) return fb(`ADC: count = Vin x 2^n / Vref. Vin=${a}V, Vref=3.3V, 10-bit: count = ?`, Math.round(a * 1023 / 3.3), `(${a} x 1023) / 3.3.`);
        if (r === 9) return fb(`${a}x + ${b}x = ${(a + b) * c}, x = ?`, c, `Combine: ${a + b}x.`);
        if (r === 10) return fb(`y = ${a}x + ${b}, x=${c}, y = ?`, a * c + b, `Substitute x=${c}.`);
        return fb(`Duty = compare/${a} x 100. compare=${b}: duty = ?%`, Math.round(b / a * 100), `(${b}/${a}) x 100.`);
      case 2:
        if (r === 0) return fb(`Sensor: y = ${a}x + ${b}, x=${c}. y = ?`, a * c + b, `Substitute x=${c}.`);
        if (r === 1) return fb(`f(x) = ${a}x + ${b}, f(${c}) - f(${d}) = ?`, a * (c - d), `Simplify: ${a}(${c} - ${d}).`);
        if (r === 2) return fb(`Scale: raw ${a * b} maps to ${a * b * c}. Slope = ?`, c, `${a * b * c} / ${a * b}.`);
        if (r === 3) return fb(`Temp: y = ${a}x + ${b}. x=${c}: temp = ?`, a * c + b, `Plug x=${c}.`);
        if (r === 4) return fb(`Parallel slope to y = ${a}x + ${b} = ?`, a, "Same slope.");
        if (r === 5) return fb(`x^2 = ${c * c}. x > 0 = ?`, c, `sqrt(${c * c}) = ${c}.`);
        if (r === 6) return fb(`Mapping: x=${b} -> y=${a * b}. f(x) = ?x`, a, `y = ${a}x.`);
        if (r === 7) return fb(`Normalize: ${a}, ${a + b}, mean=${a + b/2}. Mid = ?`, a + b / 2, `(${a} + ${a + b})/2.`);
        if (r === 8) return fb(`ReLU: x=${a - 5}. ReLU(x) = ?`, Math.max(0, a - 5), "ReLU = max(0, x).");
        if (r === 9) return fb(`Bias: y = ${a}x + ${b}. x=0 => ?`, b, "Bias = y-intercept.");
        if (r === 10) return fb(`Slope through (${a},${a*b}) and (${2*a},${2*a*b}) = ?`, b, `(${2*a*b}-${a*b})/(${2*a}-${a}).`);
        return fb(`Sigmoid(0) = ?`, 0.5, "1/(1+e^0) = 1/2.");
      case 3:
        if (r === 0) return fb(`Model: ${a * 1000} float32 params = ? KB`, (a * 1000 * 4) / 1024, `(${a}K x 4) / 1024.`);
        if (r === 1) return fb(`Conv: ${a}in, ${b}out, ${c}x${c} kernel + bias = ? params`, a * b * c * c + b, `(${a} x ${b} x ${c}^2) + ${b}.`);
        if (r === 2) return fb(`Image ${a * 10}x${b * 10} RGB888 (3B/pixel) = ? KB`, (a * 10 * b * 10 * 3) / 1024, `(${a * 10} x ${b * 10} x 3) / 1024.`);
        if (r === 3) return fb(`Register @ 0x${a}000: offset 0x${b}00 addr = ? (hex dec)`, a * 4096 + b * 256, `Base + offset.`);
        if (r === 4) return fb(`Tensor arena ${a * 10}x${b * 10}x4 bytes = ? KB`, (a * 10 * b * 10 * 4) / 1024, `(${a * 10} x ${b * 10} x 4) / 1024.`);
        if (r === 5) return fb(`Flash cap: 2^${a} bytes = ? KB`, 2 ** a / 1024, `${2 ** a} / 1024.`);
        if (r === 6) return fb(`Dense: ${a}->${b} weights = ? params`, a * b, `${a} x ${b}.`);
        if (r === 7) return fb(`SRAM: ${a}KB stack, ${b}KB heap = ? KB total`, a + b, `${a} + ${b}.`);
        if (r === 8) return fb(`Padding ${a}: input ${b}x${b} output = ? x ?`, b + 2 * a, `${b} + 2*${a}.`);
        if (r === 9) return fb(`int8 quant ${a * 100}: scale=0.${b}. Dequant = ?`, +(0.1 * b * a * 100).toFixed(1), `${a * 100} x 0.${b}.`);
        if (r === 10) return fb(`Pool ${a}x${a}, stride ${a}, input ${b * a}x${b * a}: output = ?`, b, `${b * a}/${a}.`);
        return fb(`Flash: ${a}MB = ? KB`, a * 1024, `${a} x 1024.`);
      case 4:
        if (r === 0) return fb(`sin(30)=0.5. AC peak ${a}V at 30 deg: V = ?`, a * 0.5, `${a} x 0.5.`);
        if (r === 1) return fb(`cos(60)=0.5. ${a}Hz cos wave: V at 60 deg = ${a} x ?`, 0.5, "cos(60) = 0.5.");
        if (r === 2) return fb(`tan(45)=1. Motor torque factor = ?`, 1, "tan(45) = 1.");
        if (r === 3) return fb(`sin(45) ≈ 0.71. ${a}V amplitude, V at 45 = ?`, +(a * 0.71).toFixed(2), `Amplitude x 0.71.`);
        if (r === 4) return fb(`PWM: ${a * 10}Hz, resolution ${b} bits = ? steps`, 2 ** b, `2^${b} = ${2 ** b}.`);
        if (r === 5) return fb(`sin(90)=1. Max AC voltage = ${a}V. Peak = ?`, a, "Peak amplitude.");
        if (r === 6) return fb(`cos(180) = -1. Inverted signal = ?`, -1, "Negative peak.");
        if (r === 7) return fb(`3-phase sum Vn = Va+Vb+Vc = ${a}+${b}+0 = ?`, a + b, `${a} + ${b}.`);
        if (r === 8) return fb(`sin(-30) = -0.5. Symmetry check: -sin(30) = ?`, -0.5, "sin is odd.");
        if (r === 9) return fb(`cos(-60) = 0.5. Cos even: cos(60) = ?`, 0.5, "cos is even.");
        if (r === 10) return fb(`Freq ${a * 10}Hz, period = ? ms`, Math.round(1000 / (a * 10)), "1000/f.");
        return fb(`Phase shift 180 deg = pi rad. pi ≈ ?`, 3.14, "pi ≈ 3.14.");
      case 5:
        if (r === 0) return fb(`2^${a} bytes = ? KB`, 2 ** a / 1024, `${2 ** a} / 1024.`);
        if (r === 1) return fb(`Params: ${a}K float32 = ? KB`, a * 4, `${a}K x 4 bytes.`);
        if (r === 2) return fb(`log2(${2 ** a}) = ${a}. Bits needed = ?`, a, `log2 = ${a}.`);
        if (r === 3) return fb(`10^${a} Hz = ? MHz`, 10 ** a / 1e6, `10^${a} / 1e6 = ${10 ** a / 1e6}.`);
        if (r === 4) return fb(`2^10 = 1024 = 1 ?`, 1024, "1K.");
        if (r === 5) return fb(`Memory: 2^${a} addresses, ${b} bytes each = ? total`, 2 ** a * b, `${2 ** a} x ${b}.`);
        if (r === 6) return fb(`Seq: ${a}K, ${a * 2}K, ${a * 4}K, next = ? KB`, a * 8, "Doubling capacity.");
        if (r === 7) return fb(`log10(${10 ** a}) = ?`, a, `10^${a} = ${10 ** a}.`);
        if (r === 8) return fb(`2^${a} x 2^${b} = 2^? (total bits)`, a + b, "Add exponents.");
        if (r === 9) return fb(`1 nF = 10^? F`, -9, "nano = 10^-9.");
        if (r === 10) return fb(`1 pF = 10^? F`, -12, "pico = 10^-12.");
        return fb(`2^3 = 8 = bits per ?`, 1, "8 bits = 1 byte.");
      case 6:
        if (r === 0) return fb(`PID P: Kp=${a}, err=${b}. P out = ?`, a * b, `Kp x err.`);
        if (r === 1) return fb(`Rate: temp ${a} to ${b} in ${c}s. dT/dt = ?`, +(b - a) / c, `(${b} - ${a}) / ${c}.`);
        if (r === 2) return fb(`Int${a} from 0 to ${b}: area = ?`, a * b, "Height x width.");
        if (r === 3) return fb(`f(t) = ${a}t. f(${b}) = ?`, a * b, `${a} x ${b}.`);
        if (r === 4) return fb(`Slope of y = ${a}x + ${b} = ?`, a, "Coefficient of x.");
        if (r === 5) return fb(`A ${a * 10}Hz signal T = ? ms`, Math.round(1000 / (a * 10)), "1000/f.");
        if (r === 6) return fb(`d/dx ${a}x = ? (gain)`, a, "Derivative of ax = a.");
        if (r === 7) return fb(`Accum err: ${a} + ${b} + ${c} = ?`, a + b + c, "Sum errors.");
        if (r === 8) return fb(`Delta: f(x+${c}) - f(x) for ${a}x + ${b} = ?`, a * c, "Change = a x delta x.");
        if (r === 9) return fb(`Grad: d/dw (w*x) at x=${b} = ? (with w=${a})`, b, "Derivative w.r.t w is x.");
        if (r === 10) return fb(`MSE single: (y - y_hat)^2. y=${a}, y_hat=${b}: MSE = ?`, (a - b) ** 2, `(${a} - ${b})^2.`);
        return fb(`Deriv ${a}x^2 at x=${b}: slope = ?`, 2 * a * b, "d/dx ax^2 = 2ax.");
      default:
        return fb(`${a} + ${b} = ?`, a + b, "Add.");
    }
  }
